...
\textbf{Independent verification.}\; We now confirm this by classifying partitions by their block sizes (type $\{3,1,1\}$ vs.\ type $\{2,2,1\}$).
...
\textbf{Step 0: Choose the universe.}\; The first decision in any inclusion-exclusion problem is: \emph{what is the universe?} Here we take $S$ = integers in $\{1, \ldots, 16500\}$ divisible by 11. All counting happens inside $S$.
...
\begin{tikzpicture}[scale=0.7, font=\small]
  ...
  % Labels
  \node[font=\footnotesize] at (2, -0.9) {Grid with $n=4$};
  \node[font=\scriptsize, setA!70!black] at (-1.8, 1.5) {upward};
  \draw[->, setA!70!black, thin] (-1.0, 1.35) -- (0.4, 0.8);
  \node[font=\scriptsize, setB!70!black] at (4.6, 1.5) {downward};
...
\textbf{What are we counting?}\; An ``upward triangle of side $s$'' is a triangle with the same orientation as the outer triangle, whose side spans $s$ grid units. A ``downward triangle of side $s$'' is inverted. Both types must have all three sides parallel to the outer triangle's sides.
...
% ══════════════════════════════════════════════════════════════════
\newpage
\section{Problem 12 --- Bounded Integer Solutions}
% ══════════════════════════════════════════════════════════════════
...
Remaining sum: $22 - 12 - 10 = 0$.
\quad $|A_1 \cap A_2| = \binom{2}{2} = 1$.

$A_1 \cap A_3$: $y_1 \ge 12,\; y_3 \ge 12$.
Remaining sum: $22 - 12 - 12 = -2 < 0$.
\quad $|A_1 \cap A_3| = 0$.
A negative remaining sum means the system is impossible: nonnegative variables cannot sum to a negative number, so there are $0$ solutions.

$A_2 \cap A_3$: $y_2 \ge 10,\; y_3 \ge 12$.
Remaining sum: $22 - 10 - 12 = 0$.
