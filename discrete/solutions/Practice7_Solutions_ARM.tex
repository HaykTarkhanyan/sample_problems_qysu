...
\textbf{Երկրորդ սեռի Ստիռլինգի թիվը}՝ $S(n,k)$-ն, հաշվում է $n$-էլեմենտանի բազմության տրոհումների քանակը ճիշտ $k$ հատ ոչ դատարկ, զույգ առ զույգ չհատվող ենթաբազմությունների (որոնք կոչվում են \emph{դասեր} կամ \emph{բլոկներ})։
...
\begin{problembox}
Քանի՞ եղանակով կարելի է $5$-էլեմենտանի բազմությունն տրոհել $3$ ոչ դատարկ դասերի։
\end{problembox}
...
\textbf{Անկախ ստուգում։}\; Հաստատենք սա՝ դասակարգելով տրոհումները ըստ բլոկների չափերի (տիպ $\{3,1,1\}$ և տիպ $\{2,2,1\}$)։

\textbf{Ստուգում բացահայտ թվարկմամբ։}\;
Դիցուք՝ բազմությունն է $\{a,b,c,d,e\}$։ 3 դասերի տրոհումը պետք է ունենա $\{3,1,1\}$ կամ $\{2,2,1\}$ տիպը (5-ի միակ տրոհումները 3 մասերի)։

\begin{itemize}[nosep]
  \item \textbf{Տիպ $\{3,1,1\}$՝}\;
        Ընտրում ենք 3-էլեմենտանի բլոկը՝ $\binom{5}{3} = 10$ եղանակ։
...
\begin{enumerate}[nosep]
  \item \textbf{Դեպի վեր ուղղված} եռանկյուններ (նույն կողմնորոշմամբ, ինչ սկզբնականը),
        որոնց կողմի երկարությունը $s$ է ($1 \le s \le n$)՝
        \[
          U(n,s) = \frac{(n-s+1)(n-s+2)}{2} = \binom{n-s+2}{2}։
        \]
  \item \textbf{Դեպի վար ուղղված} եռանկյուններ (շրջված կողմնորոշմամբ),
        որոնց կողմի երկարությունը $s$ է ($1 \le s \le \lfloor n/2 \rfloor$)՝
        \[
...
\]
հետևյալ սահմաններով՝ $0 \le y_1 \le 11$,\; $0 \le y_2 \le 9$,\; $0 \le y_3 \le 11$։

\textbf{Քայլ 2. Հաշվում ենք առանց վերին սահմանների։}

Առանց վերին սահմանների՝ $y_1 + y_2 + y_3 = 22$ հավասարման ոչ բացասական ամբողջ լուծումների քանակն է՝
\[
