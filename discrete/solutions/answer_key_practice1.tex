...
Describe $\overline{A},\ \overline{A\cup B},\ \overline{C},\ A\setminus C,\ C\setminus(A\cup B)$ with $U = \mathbb{Z}$, $A = \{2k \mid k \in \mathbb{Z}^+\}$ (positive evens), $B = \{2k-1 \mid k \in \mathbb{Z}^+\}$ (positive odds), $C = \{x \in \mathbb{Z} \mid x < 10\}$.

Here $A \cap B = \varnothing$ and $A \cup B = \mathbb{Z}^+$ (every positive integer is even or odd).
\begin{itemize}
...
\textbf{Part 1:} $A \subseteq B \iff \overline{B} \subseteq \overline{A} \iff A \cup B = B \iff A \cap B = A$.

\textit{Proof sketch.} Close the cycle $A \subseteq B \Rightarrow A \cup B = B \Rightarrow A \cap B = A \Rightarrow \overline{B} \subseteq \overline{A} \Rightarrow A \subseteq B$.
$(A \subseteq B) \Rightarrow (A \cup B = B)$: every element of $A$ is already in $B$, so $A \cup B = B$.
$(A \cup B = B) \Rightarrow (A \cap B = A)$: if $x \in A$ then $x \in A \cup B = B$, so $x \in A \cap B$.
$(A \cap B = A) \Rightarrow (\overline{B} \subseteq \overline{A})$: $A \cap B = A$ gives $A \subseteq B$; if $x \notin B$ then $x \notin A$, i.e.\ $\overline{B} \subseteq \overline{A}$.
$(\overline{B} \subseteq \overline{A}) \Rightarrow (A \subseteq B)$: contrapositive --- if $x \in A$ then $x \notin \overline{A}$, so $x \notin \overline{B}$, i.e.\ $x \in B$. \qed
...
Find and prove: $1 + 5 + 9 + \cdots + (4n+1)$.

The last term $4n+1$ is the $(n+1)$-th term of $1,5,9,\ldots$, so the sum has $n+1$ terms and equals $\boxed{(n+1)(2n+1)}$.

\textit{Proof.} $\displaystyle\sum_{k=1}^{n+1}(4k-3) = 4\cdot\frac{(n+1)(n+2)}{2} - 3(n+1) = (n+1)\bigl[2(n+2)-3\bigr] = (n+1)(2n+1)$.
Check: $n=1$: $1+5 = 6 = 2\cdot 3$;\; $n=2$: $1+5+9 = 15 = 3\cdot 5$. $\checkmark$ \qed

%======================================================================
\subsection*{Problem 45}
Find and prove: $-1 + 2 - 3 + 4 - \cdots - (2n-1) + 2n$.

The sum is $\boxed{n}$.

\textit{Proof.} Group in $n$ pairs: $\bigl(-(2k-1) + 2k\bigr) = 1$ for $k = 1, \ldots, n$. Hence the total is $n \cdot 1 = n$.
Check: $-1+2 = 1$;\; $-1+2-3+4 = 2$;\; $-1+2-3+4-5+6 = 3$. $\checkmark$ \qed

%======================================================================
\subsection*{Problem 46}
Find and prove: $1 - 3 + 5 - 7 + \cdots + (4n-3) - (4n-1)$.

The sum is $\boxed{-2n}$.

\textit{Proof.} Group in $n$ pairs: $\bigl((4k-3) - (4k-1)\bigr) = -2$ for $k = 1, \ldots, n$. Hence the total is $-2n$.
Check: $1-3 = -2$;\; $1-3+5-7 = -4$;\; $1-3+5-7+9-11 = -6$. $\checkmark$ \qed
...
\textit{Proof sketch.} The third term contains $A \cap \overline{A} = \varnothing$, so it vanishes. The second term is a subset of the first (it has extra constraints $C$ and $Y$), so $\text{first} \cup \text{second} = \text{first}$, by absorption. Result: $A \cap B \cap X$. \qed

\textbf{(2)} $(A \cap B \cap C) \cup (\overline{A} \cap B \cap C) \cup \overline{B} \cup \overline{C} = U$.

\textit{Proof sketch.} $(A \cap B \cap C) \cup (\overline{A} \cap B \cap C) = (A \cup \overline{A}) \cap B \cap C = B \cap C$, so the left side is $(B \cap C) \cup \overline{B} \cup \overline{C} = (B \cap C) \cup \overline{B \cap C} = U$ (De Morgan). \qed

\textbf{(3)} $(A \cap B \cap C \cap \overline{X}) \cup (\overline{A} \cap C) \cup (\overline{B} \cap C) \cup (C \cap X) = C$.

\textit{Proof sketch.} Factor out $C$: the left side is $C \cap \bigl[(A \cap B \cap \overline{X}) \cup \overline{A} \cup \overline{B} \cup X\bigr]$. With $S = A \cap B \cap \overline{X}$, note $\overline{A} \cup \overline{B} \cup X = \overline{S}$, so the bracket is $S \cup \overline{S} = U$. Hence the result is $C \cap U = C$. \qed

%======================================================================
\subsection*{Problem 54}
Prove the set identities (all true; each by element-chasing, translating $\setminus$ as $\cap\,\overline{(\cdot)}$ and $A \triangle B$ as ``$x$ in exactly one of $A, B$'').

\textbf{(1)} $A \cap (B \setminus C) = (A \cap B) \setminus C$. \\
$x \in A \land x \in B \land x \notin C$ on both sides. \checkmark
...
Given $x \in A$, $\lnot(x \in A \land x \in C)$ reduces to $x \notin C$; both sides are $x \in A \land x \in B \land x \notin C$. \checkmark
...
Given $x \notin C$, $B \setminus C$ reduces to $B$; both sides are $x \in A \land x \notin B \land x \notin C$ (cf.\ Problem 22). \checkmark

\textbf{(7)} $A \triangle \varnothing = A$. \quad $(A \setminus \varnothing) \cup (\varnothing \setminus A) = A \cup \varnothing = A$. \checkmark

\textbf{(8)} $A \triangle B = B \triangle A$. \quad ``In exactly one of $A, B$'' is symmetric in $A, B$. \checkmark

\textbf{(9)} $A \triangle (B \triangle C) = (A \triangle B) \triangle C$. \quad Both sides hold iff $x$ lies in an \emph{odd} number of $A, B, C$ (parity is associative). \checkmark
...
Both sides reduce to $(x \in A \land x \in B \land x \notin C) \lor (x \in A \land x \notin B \land x \in C)$. \checkmark
