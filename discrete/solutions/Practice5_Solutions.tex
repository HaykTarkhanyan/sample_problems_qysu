...
\textbf{Alternative (complement method).}\;
Total pairs of books among all $15+12+10=37$ books: $\binom{37}{2}$.
Subtract pairs within each group (same subject): $\binom{15}{2} + \binom{12}{2} + \binom{10}{2}$.
Result: $\binom{37}{2} - \binom{15}{2} - \binom{12}{2} - \binom{10}{2} = 666 - 105 - 66 - 45 = 450$.
...
\textbf{Example.}\; $2\,143\,658$ is a valid 7-digit number (leading digit $\neq 0$). But $0\,143\,658$ is not, because it would be a 6-digit number. The leading-digit restriction removes exactly $\frac{1}{5}$ of all valid 7-character strings (position~1 is constrained to even digits $\{0,2,4,6,8\}$, and excluding~0 removes 1 out of 5 choices).
...
\begin{center}
$\boxed{B}\;\_\;\_\;\_\;\cdots\;\_\;\_\;\boxed{B}$
\end{center}
...
% ══════════════════════════════════════════════════════════════════
\newpage
\section{Problem 10 --- Ramsey's Theorem \texorpdfstring{$R(3,3) = 6$}{R(3,3) = 6}}
% ══════════════════════════════════════════════════════════════════
...
\textbf{Why WLOG works.}\; If vertex $A$ instead has 3 blue edges, repeat the exact same argument with the roles of blue and red swapped. The problem is symmetric in the two colors, so the conclusion is the same.
...
\begin{tikzpicture}[>=Stealth, font=\small,
  ...
  % Legend
  \node[font=\scriptsize] at (3.2, 1.0) {\textcolor{setB}{\rule{8pt}{2pt}} red (friends)};
  \node[font=\scriptsize] at (3.2, 0.5) {\textcolor{setA}{- - -} blue (strangers)};

  % Brace for the 3 red neighbors
  \draw[decorate, decoration={brace, amplitude=5pt}, thick, setB!70!black]
    (60:2.7) arc (60:180:2.7);
  \node[font=\scriptsize, setB!70!black] at (120:3.3) {$\ge 3$ same colour};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[font=\small,
  ...
  \node[font=\footnotesize] at (0, -2.2) {$K_5$: edges \textcolor{setB}{red}, diagonals \textcolor{setA}{blue} --- no monochromatic triangle};
\end{tikzpicture}
\end{center}
...
% ══════════════════════════════════════════════════════════════════
\newpage
\section{Problem 12 --- Divisibility Among \texorpdfstring{$7, 77, 777, \ldots$}{7, 77, 777, ...}}
% ══════════════════════════════════════════════════════════════════
...
\textbf{Example.}\; $\underbrace{77\ldots7}_{5} - \underbrace{77\ldots7}_{3} = 77777 - 777 = 77000 = 77 \times 1000 = \underbrace{77\ldots7}_{2} \times 10^3$.
