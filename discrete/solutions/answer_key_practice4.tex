...
$f$ is total and $g$ is partial. Composition is left to right, $(f\cdot g)(x)=g(f(x))$:
\begin{itemize}[nosep]
  \item $g\cdot f$ (apply $g$ first): defined at $x$ only if $g(x)$ is defined; wherever $g(x)$ is defined, $f(g(x))$ is defined since $f$ is total. So $\operatorname{Dom}(g\cdot f)=\operatorname{Dom}(g)$.
  \item $f\cdot g$ (apply $f$ first): $f(x)$ is always defined, but $g(f(x))$ requires $f(x)\in\operatorname{Dom}(g)$. So $f\cdot g$ may be partial: defined at $x$ only if $f(x)\in\operatorname{Dom}(g)$.
\end{itemize}
...
(Left-to-right convention: $(f\cdot g)(x)=g(f(x))$, apply $f$ first.)
\begin{enumerate}[label=(\alph*), nosep]
  \item $f(x)=x^2+1$, $g(x)=x+3$:
    \[
      (f\cdot g)(x) = x^2+4, \qquad (g\cdot f)(x) = x^2+6x+10.
    \]
  \item $f(x)=\sqrt{x^2+2}$, $g(x)=x^2+3$:
    \[
      (f\cdot g)(x) = x^2+5, \qquad (g\cdot f)(x) = \sqrt{x^4+6x^2+11}.
    \]
  \item $f(x)=\frac{1}{x}$, $g(x)=2x+3$:
    \[
      (f\cdot g)(x) = \frac{2+3x}{x}\;(x\neq 0), \qquad (g\cdot f)(x) = \frac{1}{2x+3}\;(x\neq -\tfrac{3}{2}).
    \]
\end{enumerate}
...
\textbf{Inverse:} We need $4^{-1}\pmod{15}$. Since $4\cdot 4=16\equiv 1\pmod{15}$, we have $4^{-1}\equiv 4$. Thus $f^{-1}(x)\equiv 4x\pmod{15}$ (the function is its own inverse).
...
\textbf{Inverse:} $2^{-1}\equiv 8\pmod{15}$ (since $2\cdot 8=16\equiv 1$). So $f^{-1}(y)\equiv 8(y-4)\equiv 8y+13\pmod{15}$.
