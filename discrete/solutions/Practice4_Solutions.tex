...
\textbf{Two-step method for each mapping:}
\begin{enumerate}[nosep]
  \item \textbf{Domain check:} determine where the formula is defined.
  \item \textbf{Range check:} determine which outputs are actually achieved.
\end{enumerate}
...
\textbf{Sign analysis.}\; Solving $x^2 = \frac{1+2y}{y} \ge 0$ requires the numerator and denominator to have the same sign. For $y > 0$: $1+2y > 0$ automatically, so all $y > 0$ work. For $y < 0$: we need $1+2y < 0$, i.e.\ $y < -\tfrac{1}{2}$. Hence $y \in (-\infty, -\tfrac{1}{2}) \cup (0, +\infty)$, plus we must check the boundary.
...
\textbf{Domain check.}\; After computing the formula, verify where it is defined---the domain of a composition $(g \circ f)(x)$ requires $x \in \operatorname{Dom}(f)$ and $f(x) \in \operatorname{dom}(g)$.
...
\bigskip
\textbf{Note.}\; Here the inverse relation happens to be a function because each mapping is bijective onto its range. In general, the inverse \emph{relation} always exists, but it is a \emph{function} only when the original mapping is injective.
...
\textbf{Efficient tests for injectivity and surjectivity:}
\begin{itemize}[nosep]
  \item $|x|$: symmetry ($f(x) = f(-x)$) immediately shows non-injectivity.
  \item $x^2 + c$: parabola has a minimum, so not surjective onto $\mathbb{R}$.
  \item $x^3 + c$: strictly monotone, hence injective; end behavior gives surjectivity.
  \item $x + |x|$: rewrite piecewise first, then check each piece.
  \item $x^3 - 4x$: look for repeated outputs (local max/min) and use end behavior for surjectivity.
\end{itemize}
...
\begin{tikzpicture}[scale=0.55, >=Stealth]
  ...
  % Annotations
  \node[font=\scriptsize, setB!70!black, align=center] at (-3, 2.5) {$y=0$ hits\\3 points:\\not injective};
...
\bigskip
\textbf{Concrete example.}\; If $f \colon \{1,2,3,4\} \to \{a,b\}$ with $f(1) = f(3) = a$ and $f(2) = f(4) = b$, then:
\begin{itemize}[nosep]
  \item The equivalence class of $1$ is $f^{-1}(\{a\}) = \{1, 3\}$.
  \item The equivalence class of $2$ is $f^{-1}(\{b\}) = \{2, 4\}$.
  \item The partition is $\bigl\{\{1,3\},\;\{2,4\}\bigr\}$.
\end{itemize}

% ══════════════════════════════════════════════════════════════════
\newpage
\section{Problem 12 --- \texorpdfstring{$f(x) = \frac{x}{2x+1}$}{f(x)=x/(2x+1)} on \texorpdfstring{$\Q^+$}{Q+}: Injective but Not Surjective}
% ══════════════════════════════════════════════════════════════════
...
\textbf{Intuition.}\; Rewriting $f(x) = \dfrac{1}{2 + 1/x}$ shows that as $x$ increases on $\mathbb{Q}^+$, the denominator $2 + 1/x$ decreases, so $f(x)$ increases. A strictly increasing function is always injective.
...
% ══════════════════════════════════════════════════════════════════
\newpage
\section{Problem 19 --- \texorpdfstring{$f(A \cap B) = f(A) \cap f(B)$}{f(A∩B)=f(A)∩f(B)} Iff \texorpdfstring{$f$}{f} Is Injective}
% ══════════════════════════════════════════════════════════════════
...
\begin{tikzpicture}[>=Stealth, font=\small, scale=0.9]
  ...
  \begin{scope}[xshift=0cm]
    ...
    \node[font=\tiny, align=center] at (1.0, 0.0) {$A \cap B = \varnothing$\\but $f(A)\cap f(B) = \{y\}$};
  \end{scope}
\end{tikzpicture}
\end{center}
\end{theorybox}
...
\textbf{Why this matters.}\; If $f$ is not injective---say $f(1) = f(2) = y$---then $f(\{1\}) \cap f(\{2\}) = \{y\} \neq \varnothing$, but $\{1\} \cap \{2\} = \varnothing$. So $f(A \cap B) \neq f(A) \cap f(B)$ in general. Injectivity is exactly the condition that prevents this.
...
\begin{itemize}[nosep]
  \item $A \cap B = \varnothing$ (since $x_1 \neq x_2$), so $f(A \cap B) = f(\varnothing) = \varnothing$.
...
(i.e.\ $X_1 \cap X_2 = \varnothing$, $X_1 \neq \varnothing$, $X_2 \neq \varnothing$)
to have $f(X_1) = f(X_2)$?
\end{problembox}
...
\begin{problembox}
Is it true that for $f : A \to B$, if $Y_1 \cap Y_2 = \varnothing$, then
...
\begin{proof}
Suppose $Y_1 \cap Y_2 = \varnothing$. We use the identity $f^{-1}(Y_1 \cap Y_2) = f^{-1}(Y_1) \cap f^{-1}(Y_2)$.

Since $Y_1 \cap Y_2 = \varnothing$:
\[
...
\end{itemize}
So $f(x) \in Y_1 \cap Y_2 = \varnothing$, which is a contradiction since no element belongs to the empty set.
\end{proof}

\begin{answerbox}
\textbf{Yes.} If $Y_1 \cap Y_2 = \varnothing$, then $f^{-1}(Y_1) \cap f^{-1}(Y_2) = \varnothing$.
This follows from the fact that preimages preserve intersections.
\end{answerbox}

\bigskip
\textbf{Contrast with images.}\; Preimages \emph{always} preserve intersections: $f^{-1}(C \cap D) = f^{-1}(C) \cap f^{-1}(D)$. Images do \emph{not}, unless $f$ is injective (see Problem~19). This asymmetry is a fundamental feature of functions.

% ══════════════════════════════════════════════════════════════════
\newpage
\section{Problem 25 --- \texorpdfstring{$f(f^{-1}(Y)) \subseteq Y$}{f(f⁻¹(Y)) ⊆ Y}}
% ══════════════════════════════════════════════════════════════════
...
\bigskip
\textbf{Companion fact.}\; The reverse inclusion $X \subseteq f^{-1}\bigl(f(X)\bigr)$ always holds, with equality if and only if $f$ is injective. Together with this problem's result $f\bigl(f^{-1}(Y)\bigr) \subseteq Y$, these two inclusions capture how images and preimages interact.
