...
% ── Title ─────────────────────────────────────────────────────────
\title{\textbf{Գործնական 6 — Զուգորդություններ}\\[4pt]
...
\begin{tikzpicture}[scale=0.6, font=\small]
  % Pascal Triangle overlay
  \foreach \n in {0,...,4} {
    \foreach \k in {0,...,\n} {
      \node[circle, fill=gray!10, inner sep=1pt, minimum size=12pt]
        at (\k*1.2 - \n*0.6, -\n*0.8) {\tiny $\binom{\n}{\k}$};
    }
  }
\end{tikzpicture}
...
\[
  \text{Ընդհանուր ճանապարհներ} = 35 \times 70 = 2450 \text{։}
\]
...
\begin{tikzpicture}[scale=0.5]
  \foreach \x in {0.5, 1.5, 2.5, 3.5, 5.5, 6.5} \node at (\x, 0) {$\star$};
  \draw[thick] (4, -0.4) -- (4, 0.4);
  \draw[thick] (7, -0.4) -- (7, 0.4);
  \node at (8.5, 0) {$\star \dots$};
\end{tikzpicture}
\end{center}
...
\medskip
\textbf{Բնական թվի տրոհումներ (Integer partitions)} (չկարգավորված).\;
...
Կարգը էական չէ, այսինքն՝ $4+1+1$ և $1+4+1$ համարվում են \emph{նույն} տրոհումը (բնական թվի տրոհումներ)։

\begin{center}
\begin{tikzpicture}[scale=0.3]
  \foreach \x in {0,1,2,3} \draw[fill=gray!20] (\x,2) rectangle ++(1,1);
  \foreach \x in {0} \draw[fill=gray!20] (\x,1) rectangle ++(1,1);
  \foreach \x in {0} \draw[fill=gray!20] (\x,0) rectangle ++(1,1);
  \node[right] at (5, 1.5) {$4+1+1$};
\end{tikzpicture}
\end{center}
\end{theorybox}

\begin{problembox}
Գտեք 6 թիվը երեք գումարելիների տրոհելու տարբերակների քանակը։
\end{problembox}

\subsection*{Լուծում}

\textbf{Խնդրի պայմանի ճշգրտում.}\; «Տրոհել երեք գումարելիների» կարող է նշանակել կարգավորված (կոմպոզիցիաներ) կամ չկարգավորված (բուն տրոհումներ)։ Մենք կլուծենք երկու տարբերակն էլ։
...
\begin{theorybox}[Բազմանդամային գործակից / Կրկնություններով տեղափոխություններ]
Եթե ունենք $n$ օբյեկտ, որտեղ կան $k$ տարբեր տեսակներ $n_1, n_2, \ldots, n_k$
քանակներով (այնպես որ $n_1 + n_2 + \cdots + n_k = n$), ապա տարբեր
դասավորությունների քանակը հավասար է \textbf{Բազմանդամային գործակցին}․
\[
  \binom{n}{n_1,\, n_2,\, \ldots,\, n_k}
  = \frac{n!}{n_1!\cdot n_2!\cdots n_k!} \text{։}
\]

\medskip
\textbf{Իմաստը.}\;
Սկսում ենք $n!$ դասավորություններից (կարծես բոլոր տառերը տարբեր են),
ապա բաժանում ենք յուրաքանչյուր միատեսակ տառերի խմբի ներքին տեղափոխությունների քանակին ($n_i!$)։
\end{theorybox}
...
\medskip
\textbf{Քայլ 2. Կիրառել բազմանդամային գործակիցը.}
