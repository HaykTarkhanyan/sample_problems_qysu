...
\textbf{Why multiplication.}\; Every valid full path corresponds to exactly one first segment and one second segment. Since the choices are independent, the multiplication principle applies.
...
\textbf{Ambiguity warning.}\; ``Partition into three summands'' can mean ordered (compositions) or unordered (partitions). We solve both interpretations below.
...
\textbf{First step.}\; Count the letters yourself before reading the table---this is a useful habit that catches transcription errors.
