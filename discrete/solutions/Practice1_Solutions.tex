...
% ══════════════════════════════════════════════════════════════════
\section{Problem 2 --- Membership with $\varnothing$ and $\{\varnothing\}$}
% ══════════════════════════════════════════════════════════════════
...
The set $\{\varnothing\}$ is \textbf{not} empty---it has one element, namely $\varnothing$ itself.
...
\begin{tikzpicture}[
  ...
  \node[box, fill=gray!8] (e0)
    {$\varnothing = \{\,\}$\\[-1pt]\footnotesize 0 elements};
  \node[box, fill=setA!8, right=1.6cm of e0] (e1)
    {$\{\varnothing\}$\\[-1pt]\footnotesize 1 element: $\varnothing$};
...
\medskip
\begin{tcolorbox}[colback=blue!3, colframe=setA!60!black, arc=2mm, left=5pt, right=5pt, top=5pt, bottom=5pt, title={\small Box Analogy: How $\in$ Works}]
Think of every set as a \textbf{box}. The $\in$ operator opens the box and looks at what is \emph{directly} inside---it only looks \textbf{one box deep}. It never opens inner boxes.

\begin{itemize}[nosep]
  \item $\varnothing$ is an \textbf{empty box}---you open it and find nothing inside.
        Therefore $|\varnothing| = 0$.
  \item $\{\varnothing\}$ is a \textbf{box containing one item}: another (empty) box.
        You open $\{\varnothing\}$ and see one thing sitting there---the empty box $\varnothing$.
        Therefore $|\{\varnothing\}| = 1$.
\end{itemize}

\medskip
This is why $\varnothing \neq \{\varnothing\}$: an empty box is not the same as a box that contains an empty box, just as an empty envelope is not the same as an envelope containing another empty envelope.

When you test ``$x \in S$\,?'', open the box $S$, look at the items directly visible (one level deep), and check whether $x$ is one of those items. \textbf{Never unwrap items inside items.}
\end{tcolorbox}

\medskip
\textbf{Method.}\; List the elements of each outer set first: $\{\varnothing,\;\{\varnothing\}\}$, $\{\{\varnothing\}\}$. Then check whether the left-hand object matches any listed element.
...
\begin{tabular}{lll}
\toprule
\textbf{Left object} & \textbf{Elements of outer set} & \textbf{Match?} \\
\midrule
$\varnothing$ & $\varnothing,\;\{\varnothing\}$ & Yes --- \textbf{TRUE} \\
$\{\varnothing\}$ & $\{\varnothing\}$ & Yes --- \textbf{TRUE} \\
$\varnothing$ & $\{\varnothing\}$ & No ($\varnothing \neq \{\varnothing\}$) --- \textbf{FALSE} \\
\bottomrule
\end{tabular}
\end{center}
...
\;---\; \textbf{TRUE.}
The set has two elements: $\varnothing$ and $\{\varnothing\}$.
The object $\varnothing$ is explicitly one of them.

\textbf{2.2.}\; $\{\varnothing\} \in \{\{\varnothing\}\}$
\;---\; \textbf{TRUE.}
The set $\{\{\varnothing\}\}$ has exactly one element: $\{\varnothing\}$.
The object on the left is $\{\varnothing\}$, which matches.

\textbf{2.3.}\; $\varnothing \in \{\{\varnothing\}\}$
\;---\; \textbf{FALSE.}
The only element is $\{\varnothing\}$.
We need $\varnothing = \{\varnothing\}$, but $|\varnothing|=0$ while $|\{\varnothing\}|=1$,
...
\begin{tcolorbox}[colback=yellow!5, colframe=stmtFrame!80!black, arc=2mm, left=5pt, right=5pt, top=5pt, bottom=5pt, title={\small Real-World Analogy: Level Jumps}]
\textbf{A person, a committee, and a task force.}

Suppose Professor~$A$ is a \emph{member} of the Curriculum Committee~$B$, and the Curriculum Committee~$B$ is a \emph{member} of the University Task Force~$C$ (a body whose members are entire committees, not individual people).

\begin{itemize}[nosep]
  \item $A \in B$: the professor sits on the committee.
  \item $B \in C$: the committee belongs to the task force.
  \item $A \notin C$: the professor is \textbf{not} individually a member of the task force---only whole committees are members of the task force.
\end{itemize}

The key insight: $\in$ crosses exactly \textbf{one level} of containment. Two $\in$-steps cross \emph{two} levels, so the individual ($A$) is buried two levels deep inside $C$ but is not a \emph{direct} member of $C$.
\end{tcolorbox}
...
\medskip
\textbf{Contrast.}\; The conclusion $A \notin C$ is not automatic---it can fail.
Let $A = 1$, $B = \{1\}$, $C = \{1, \{1\}\}$.
Then $A \in B$ ($1 \in \{1\}$), $B \in C$ ($\{1\} \in \{1, \{1\}\}$),
and also $A \in C$ ($1 \in \{1, \{1\}\}$).
So $\in$ is \emph{sometimes} transitive by accident, but not always.
...
\begin{tcolorbox}[colback=gray!3, colframe=gray!60, arc=2mm, left=5pt, right=5pt, top=5pt, bottom=5pt, title={\small Color Guide: Seeing the Nesting Levels}]
When sets contain other sets as elements, it helps to color-code the levels:
\begin{itemize}[nosep]
  \item \textcolor{blue}{Blue} = plain numbers (lowest level objects).
  \item \textcolor{red}{Red braces} = set-braces of an inner set that is itself an \emph{element} of an outer set.
  \item Black braces = the outer set's own boundary.
\end{itemize}

\medskip
For example, $C = \bigl\{\textcolor{red}{\{}\textcolor{blue}{1,2}\textcolor{red}{\}},\;\textcolor{blue}{3}\bigr\}$ has two elements:
the set $\textcolor{red}{\{}\textcolor{blue}{1,2}\textcolor{red}{\}}$ (an inner set, marked in red braces) and the number $\textcolor{blue}{3}$ (a plain number in blue).
...
\]
In each case, the \textcolor{red}{red-braced} item is a set-element (one level up), while the \textcolor{blue}{blue} items are number-elements.
\end{tcolorbox}
...
\begin{itemize}[nosep]
  \item $A, B$: sets of \emph{numbers} (elements are $1, 2$).
  \item $C, D, E$: sets whose elements \emph{include a set} ($\{1,2\}$ is an element alongside numbers like $3, 4, 5$).
\end{itemize}
The $\in$-step from $B$ to $C$ is where the level jump happens.
...
\begin{tikzpicture}[scale=0.82]
  ...
  \begin{scope}[xshift=5.5cm]
    ...
    \node[font=\scriptsize, gray] at (0, 0) {excluded};
...
\textbf{Translate each operation into words first:}
\begin{itemize}[nosep]
  \item $\comp{A}$: integers that are \textbf{not} positive even.
  \item $A \symdiff B$: integers in \textbf{exactly one} of $A, B$.
  \item $\comp{C}$: integers that are \textbf{not} less than $10$.
  \item $A \setminus C$: positive evens that are \textbf{not} in $C$ (i.e.\ $\ge 10$).
  \item $C \setminus (A \symdiff B)$: integers $< 10$ that are \textbf{not} positive.
\end{itemize}
...
\textbf{(b)\; $A \symdiff B$:}\;
Since $A \cap B = \varnothing$ (no integer is both positive-even and positive-odd):
\[
  A \symdiff B = A \cup B = \{1, 2, 3, 4, \ldots\}
\]
Since every positive integer is either even or odd, and never both, $A \cup B = \Z^+$ and $A \cap B = \varnothing$.
\[
  \boxed{A \symdiff B = \Z^+}
\]
...
This is a compound operation---evaluate it step by step:

\begin{enumerate}[label=\textbf{Step \arabic*:}, leftmargin=*, nosep, itemsep=4pt]
  \item \textbf{Evaluate the inner parentheses:}\;
        $A \symdiff B$. From part~(b) above, we already know
        $\boxed{A \symdiff B = \Z^+}$ (all positive integers).

  \item \textbf{Substitute back into the outer operation:}\;
        $C \setminus (A \symdiff B) = C \setminus \Z^+$.
        This means: take all elements of $C$ (integers $< 10$) and
        remove every positive integer.

  \item \textbf{Compute the result:}\;
        The elements of $C$ that are \emph{not} positive are
        $\ldots, -2, -1, 0$.
\end{enumerate}
\[
...
\begin{tcolorbox}[colback=theoryBg, colframe=theoryFrame!80!black, arc=2mm, left=5pt, right=5pt, top=5pt, bottom=5pt, title={\small Visual Intuition: Why These Conditions Are Equivalent}]
Before diving into the formal proof, consider this Venn diagram showing $A \subseteq B$ (i.e.\ $A$ sits entirely inside $B$):
...
\begin{itemize}[nosep]
  \item \textbf{Why $A \cup B = B$?}\; Since $A$ is already inside $B$, adding $A$ to the union contributes nothing new. The shaded region of $A \cup B$ is just $B$.
  \item \textbf{Why $A \cap B = A$?}\; Every element of $A$ is also in $B$ (because $A$ is inside $B$), so intersecting with $B$ keeps all of $A$. Nothing from $A$ gets removed.
\end{itemize}

This picture makes all three conditions feel like the same statement in different words. The formal cyclic-implication proof below confirms this rigorously.
\end{tcolorbox}
...
\smallskip
\textit{Key idea: if $A$ is already inside $B$, adding $A$ to the union changes nothing, and intersecting with $B$ keeps all of $A$.}
...
% ---- Part 2 ----
\subsection*{Part 2:\; $A \cap B = \varnothing \;\iff\; A \subseteq \comp{B} \;\iff\; B \subseteq \comp{A}$}
...
\begin{tikzpicture}[scale=0.72]
  ...
  \node[font=\footnotesize] at (0,-2.3) {Disjoint: $A \cap B = \varnothing$};
  \node[font=\scriptsize, gray] at (0, 0) {$\varnothing$};
\end{tikzpicture}
\end{center}

\smallskip
\textit{Key idea: disjointness means every element of $A$ lies outside $B$, i.e.\ in $\comp{B}$.}

\smallskip
\textbf{$(a) \Rightarrow (b)$:}\;
$A \cap B = \varnothing \;\Longrightarrow\; A \subseteq \comp{B}$.

Let $x \in A$. If $x \in B$, then $x \in A \cap B = \varnothing$---contradiction.
...
\smallskip
\textit{Key idea: if $A$ and $B$ together cover $U$, then anything outside $A$ must be in $B$.}
...
\smallskip
\textbf{Set-language reading.}\; Distributing ``outside $C$'' across ``in $A$ or in $B$'' is the set version of the logical law $p \land (q \lor r) \iff (p \land q) \lor (p \land r)$.

\medskip
\begin{tcolorbox}[colback=theoryBg, colframe=theoryFrame!80!black, arc=2mm, left=5pt, right=5pt, top=5pt, bottom=5pt, title={\small Region Numbering Method --- Empirical Verification}]
\textbf{Technique.}\; In a three-set Venn diagram there are $2^3 = 8$ regions.
Number them by which sets they belong to:

\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{cl}
\toprule
\textbf{Region} & \textbf{Membership} \\
\midrule
1 & $A$ only \\
2 & $B$ only \\
3 & $C$ only \\
...
6 & $B \cap C$ only (not $A$) \\
7 & $A \cap B \cap C$ \\
8 & None ($\comp{A} \cap \comp{B} \cap \comp{C}$) \\
\bottomrule
\end{tabular}
\end{center}

Now express each side by its region numbers:
\begin{itemize}[nosep]
  \item $A \cup B = \{1, 2, 4, 5, 6, 7\}$
  \item $(A \cup B) \setminus C$: remove regions in $C$ ($\{3,5,6,7\}$)
...
Both sides give exactly regions $\{1, 2, 4\}$, confirming the identity empirically.

\medskip
\textit{This method does not replace a proof, but it is an excellent way to build confidence or catch errors before writing one.}
\end{tcolorbox}
...
\begin{tcolorbox}[colback=gray!3, colframe=gray!60, arc=2mm, left=5pt, right=5pt, top=5pt, bottom=5pt, title={\small Intermediate Shading Steps --- Building the Venn Diagram}]
To build the Venn diagram for $A \cap (B \symdiff C)$ from scratch, follow these stages:

\begin{enumerate}[label=\textbf{Stage \arabic*:}, leftmargin=*, nosep, itemsep=4pt]
  \item \textbf{Shade $B \symdiff C$:}\;
        Shade all regions that belong to exactly one of $B$ or $C$
        (i.e.\ $B$-only and $C$-only regions, but \emph{not} the $B \cap C$ overlap).
      Using the region numbering from the previous problem (8 regions for three sets):
      $B$ occupies regions $\{2,4,6,7\}$ and $C$ occupies regions $\{3,5,6,7\}$.
      Therefore $B\setminus C = \{2,4\}$ and $C\setminus B = \{3,5\}$, so
      $B \symdiff C = (B\setminus C) \cup (C\setminus B)$ covers regions $\{2,3,4,5\}$.

  \item \textbf{Identify $A$:}\;
        The regions inside $A$ are $\{1, 4, 5, 7\}$.

  \item \textbf{Intersect---highlight the overlap:}\;
        $A \cap (B \symdiff C)$ is the intersection of stages 1 and 2:
        $\{2, 3, 4, 5\} \cap \{1, 4, 5, 7\} = \{4, 5\}$.
        These are exactly region~4 ($A \cap B \cap \comp{C}$) and
        region~5 ($A \cap C \cap \comp{B}$)---the two shaded regions in the
        diagram above.
\end{enumerate}

The same two regions emerge from the RHS: $(A \cap B) \symdiff (A \cap C) = \{4, 7\} \symdiff \{5, 7\} = \{4, 5\}$.
\end{tcolorbox}
...
\medskip
\textbf{Verification by truth table.}\;
Every element belongs to some combination of $A, B, C$. The table below checks that LHS and RHS agree in all cases.

\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{ccc|c|c}
\toprule
$x \in A$ & $x \in B$ & $x \in C$ & LHS: $A \cap (B \symdiff C)$ & RHS: $(A \cap B) \symdiff (A \cap C)$ \\
\midrule
...
\end{tabular}
\end{center}
All rows match, confirming the identity.
...
\begin{tcolorbox}[colback=yellow!5, colframe=stmtFrame!80!black, arc=2mm, left=5pt, right=5pt, top=5pt, bottom=5pt, title={\small Concrete Mini-Example: Why $A \neq B$ Breaks Equality}]
Let $A = \{1, 2\}$ and $B = \{3\}$. Then:
\[
  A \times B = \{(1,3),\;(2,3)\}, \qquad B \times A = \{(3,1),\;(3,2)\}.
\]

Visualise these as grids:
\begin{center}
\begin{tikzpicture}[scale=0.7, font=\small]
  % A x B: 2 columns x 1 row
  \begin{scope}
    \node[font=\footnotesize\bfseries] at (1.5, 2.0) {$A \times B$: $2 \times 1$ grid};
    \draw[thick, gray!50] (0.5, 0.5) grid (2.5, 1.5);
    \fill[setA!30] (0.5,0.5) rectangle (2.5,1.5);
    \draw[thick, gray] (0.5, 0.5) grid (2.5, 1.5);
    \node[font=\scriptsize] at (1.0, 1.0) {$(1,3)$};
    \node[font=\scriptsize] at (2.0, 1.0) {$(2,3)$};
    \node[font=\tiny, below] at (1.0, 0.4) {$1$};
    \node[font=\tiny, below] at (2.0, 0.4) {$2$};
    \node[font=\tiny, left] at (0.4, 1.0) {$3$};
  \end{scope}

  \node[font=\Large] at (4.0, 1.0) {$\neq$};

  % B x A: 1 column x 2 rows
  \begin{scope}[xshift=5.5cm]
    \node[font=\footnotesize\bfseries] at (1.0, 2.0) {$B \times A$: $1 \times 2$ grid};
    \fill[setB!25] (0.5,0.5) rectangle (1.5,2.5);
    \draw[thick, gray] (0.5, 0.5) grid (1.5, 2.5);
    \node[font=\scriptsize] at (1.0, 1.0) {$(3,2)$};
    \node[font=\scriptsize] at (1.0, 2.0) {$(3,1)$};
    \node[font=\tiny, below] at (1.0, 0.4) {$3$};
    \node[font=\tiny, left] at (0.4, 1.0) {$2$};
    \node[font=\tiny, left] at (0.4, 2.0) {$1$};
  \end{scope}
\end{tikzpicture}
\end{center}

The grids have different ``shapes'' ($2 \times 1$ vs.\ $1 \times 2$), and the pairs themselves are different: $(1,3) \neq (3,1)$. So $A \times B \neq B \times A$ whenever $A \neq B$ (and both are non-empty).
\end{tcolorbox}

\subsection*{Solution}

\textbf{Hidden assumption.}\; We first assume $A \neq \varnothing$ and $B \neq \varnothing$, because if either is empty, both products are $\varnothing$ and the equality holds trivially without $A = B$. (Example: $A = \varnothing$, $B = \{1\}$.)
...
\textbf{$A \subseteq B$:}\;
Let $a \in A$. Since $B \neq \varnothing$, pick some $b \in B$.
\[
...
\textbf{$B \subseteq A$:}\;
Let $b \in B$. Since $A \neq \varnothing$, pick some $a \in A$.
\[
...
\[
  \boxed{A \subseteq B \;\land\; B \subseteq A \;\Longrightarrow\; A = B.}
\]
\qed

\textit{Note: the non-emptiness assumption was used above to ``pick $b \in B$'' and ``pick $a \in A$''.}
...
\textbf{Template method.}\; To prove any Cartesian-product identity, always start with ``Let $(x,y)$ be arbitrary'' and unpack $(x,y) \in \text{LHS}$. The ordered-pair structure ensures the proof reduces to a propositional-logic equivalence.

\medskip
\begin{tcolorbox}[colback=gray!3, colframe=gray!60, arc=2mm, left=5pt, right=5pt, top=5pt, bottom=5pt, title={\small Vertical Truth Table --- Verifying the Propositional Equivalences}]
Each identity reduces to a propositional equivalence.
Let $P = (x \in A)$, $Q = (y \in B)$, $R = (y \in C)$.
The table below stacks the cases vertically for easier reading:

\begin{center}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{ccc|ccc}
\toprule
 & & & \textbf{Identity 1} & \textbf{Identity 3} & \textbf{Identity 5} \\
$P$ & $Q$ & $R$
  & $P \land (Q \lor R)$ & $P \land Q \land R$ & $P \land Q \land \lnot R$ \\
 & &
  & \;$\stackrel{?}{=}$\; $(P \land Q) \lor (P \land R)$
  & \;$\stackrel{?}{=}$\; $(P \land Q) \land (P \land R)$
  & \;$\stackrel{?}{=}$\; $(P \land Q) \land \lnot(P \land R)$ \\
\midrule
\multicolumn{3}{l|}{\textbf{Row 1:}\; $P{=}\mathrm{T},\;Q{=}\mathrm{T},\;R{=}\mathrm{T}$}
  & T $=$ T \checkmark & T $=$ T \checkmark & F $=$ F \checkmark \\[2pt]
\multicolumn{3}{l|}{\textbf{Row 2:}\; $P{=}\mathrm{T},\;Q{=}\mathrm{T},\;R{=}\mathrm{F}$}
  & T $=$ T \checkmark & F $=$ F \checkmark & T $=$ T \checkmark \\[2pt]
\multicolumn{3}{l|}{\textbf{Row 3:}\; $P{=}\mathrm{T},\;Q{=}\mathrm{F},\;R{=}\mathrm{T}$}
  & T $=$ T \checkmark & F $=$ F \checkmark & F $=$ F \checkmark \\[2pt]
\multicolumn{3}{l|}{\textbf{Row 4:}\; $P{=}\mathrm{T},\;Q{=}\mathrm{F},\;R{=}\mathrm{F}$}
  & F $=$ F \checkmark & F $=$ F \checkmark & F $=$ F \checkmark \\[2pt]
\multicolumn{3}{l|}{\textbf{Row 5:}\; $P{=}\mathrm{F},\;Q{=}\mathrm{T},\;R{=}\mathrm{T}$}
  & F $=$ F \checkmark & F $=$ F \checkmark & F $=$ F \checkmark \\[2pt]
\multicolumn{3}{l|}{\textbf{Row 6:}\; $P{=}\mathrm{F},\;Q{=}\mathrm{T},\;R{=}\mathrm{F}$}
  & F $=$ F \checkmark & F $=$ F \checkmark & F $=$ F \checkmark \\[2pt]
\multicolumn{3}{l|}{\textbf{Row 7:}\; $P{=}\mathrm{F},\;Q{=}\mathrm{F},\;R{=}\mathrm{T}$}
  & F $=$ F \checkmark & F $=$ F \checkmark & F $=$ F \checkmark \\[2pt]
\multicolumn{3}{l|}{\textbf{Row 8:}\; $P{=}\mathrm{F},\;Q{=}\mathrm{F},\;R{=}\mathrm{F}$}
  & F $=$ F \checkmark & F $=$ F \checkmark & F $=$ F \checkmark \\
\bottomrule
\end{tabular}
\end{center}

All 8 rows match for each identity, confirming the propositional equivalences used in the proofs below.
\end{tcolorbox}
