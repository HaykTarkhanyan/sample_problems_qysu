...
\begin{tikzpicture}[scale=0.82]
  ...
  \begin{scope}[xshift=5.5cm]
    ...
    \node[font=\scriptsize] at (0, 0) {\color{gray}$\varnothing$};
...
\begin{enumerate}[label=\textbf{Stage \arabic*:}, leftmargin=*, nosep, itemsep=4pt]
  \item \textbf{Shade $B \symdiff C$:}\;
        ...
        This gives regions $\{2, 4, 6\}$ in $B$-only and $\{3, 5, 6\}$ in $C$-only:
        specifically regions $\{2, 4\}$ (in $B$ not $C$) and $\{3, 5\}$ (in $C$ not $B$).
        So $B \symdiff C$ covers regions $\{2, 3, 4, 5\}$.
