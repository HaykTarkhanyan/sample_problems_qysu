...
Number of ways to write 6 as an ordered sum of three non-negative summands, $x_1+x_2+x_3=6$ (this is in the sheet's ``combinations with repetition'' section). By stars and bars:
\[ \binom{6+3-1}{3-1} = \binom{8}{2} = \boxed{28}. \]
(If the parts are required to be $\ge 1$ it is $\binom{5}{2}=10$; the \emph{unordered} partitions $4{+}1{+}1,\,3{+}2{+}1,\,2{+}2{+}2$ number $3$.)
...
Distinct rearrangements of the Armenian word for ``mathematics'' (10 letters, \textit{matematika} by sound). In Armenian script the two ``t'' sounds are written with \emph{different} letters, so the only repeats are m ($\times 2$) and a ($\times 3$); the other five letters are all distinct.

$\boxed{\frac{10!}{2!\,3!} = \frac{3{,}628{,}800}{12} = 302{,}400}$
