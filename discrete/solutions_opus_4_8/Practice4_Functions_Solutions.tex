...
\begin{axis}[
  ...
  ytick={0.5,1}, yticklabels={$\tfrac12$,$1$}, xtick={0,2,4,6,8,10},
  every axis x label/.style={at={(ticklabel* cs:1.0)},anchor=west},
  every axis y label/.style={at={(ticklabel* cs:1.0)},anchor=south}]
...
\addplot[only marks, mark=*, mark size=1.5pt, armOrange] coordinates {(0,0.5)};
% shade the unreachable band above 1/2 lightly via a thick band label
...
visible. Random finite tests confirm equality for an injective $f$ and the inclusion (only) for a
non-injective $f$.
...
\begin{problembox}[22]
Is it true that for $f:A\to B$ and $X_1,X_2\subseteq A$ with $X_1\cap X_2=\varnothing$
(disjoint, nonempty) one must have $f(X_1)=f(X_2)$? More usefully: \emph{can} two disjoint nonempty
sets have equal images?
\end{problembox}
...
\textbf{Step 4 - Always check on finite data.} 3000 random finite trials with $f(x)=x^2$ confirmed
both the identity $f^{-1}(Y_1)\cap f^{-1}(Y_2)=f^{-1}(Y_1\cap Y_2)$ and the disjoint consequence.
...
strictness is forced. 3000 random finite trials confirmed $f(f^{-1}(Y))\subseteq Y$ with no
violations.
